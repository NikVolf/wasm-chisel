...
impl ChiselDriver {
    ...
    pub fn execute_module(
        ...
        // TODO: Verify correct translate method is used on each module
        let result = match name.as_str() {
            ...
            other => {
                return Err(DriverError::ModuleNotFound(name.clone(), other.to_string()));
            }
        };
        Ok(result)
    }
...
mod tests {
    ...
    fn execute_module_smoke() {
        let tmp_path = PathBuf::from("test.wasm.tmp");

        let module = Module::default().to_bytes().expect("Cannot fail");
        write(&tmp_path, module).expect("Must not fail");
        ...
        config.rulesets_mut()[0]
            .1
            .options_mut()
            .insert("file".to_string(), "test.wasm.tmp".to_string());
        ...
        remove_file(tmp_path).expect("Must not fail");
